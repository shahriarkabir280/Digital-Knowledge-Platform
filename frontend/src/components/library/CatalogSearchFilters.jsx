...
import { Filter, X, RefreshCw, Loader2 } from 'lucide-react'
        ...
        <div className={compact ? 'flex gap-1.5 flex-wrap' : 'grid gap-1'}>
          {['', ...itemTypes].map((t) => (
            <button
              ...
              className={`
                px-2.5 py-1 rounded-full text-[13px] font-semibold border transition-colors text-left
                ${(filters.item_type ?? '') === t
                  ? 'bg-accent text-white border-accent'
                  : 'bg-card border-border text-foreground hover:border-accent/50 hover:bg-muted/30'
                }
      ...
      {activeCount > 0 && (
        <div className={compact ? 'flex items-end' : ''}>
          ...
          >
            ...
            <Badge className="text-[11px] ml-0.5 bg-accent/10 text-accent border-accent/20 border">{activeCount}</Badge>
          </Button>
        </div>
      )}
    </div>
      ...
      <div className="rounded-xl border border-border bg-card/50 p-3">
        {(loading || activeCount > 0) && (
          <div className="flex items-center gap-2 mb-3">
            ...
            {activeCount > 0 && (
              <Badge className="text-[13px] bg-accent/10 text-accent border-accent/20 border">{activeCount} active</Badge>
            )}
          </div>
        )}
        {filterContent}
    ...
    <div className="rounded-xl border border-border bg-card/50 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
          ...
          {loading && <Loader2 size={11} className="animate-spin text-muted-foreground" />}
          {activeCount > 0 && (
            <Badge className="text-[11px] bg-accent/10 text-accent border-accent/20 border">{activeCount}</Badge>
          )}
        </div>
